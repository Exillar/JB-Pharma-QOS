...
class _DocxHelper:
    ...
    def _add_picture_autofit_bounds(
        ...
        *,
        ...
        """
        Add picture while respecting both width and height bounds.
        Bounds are in EMUs (914400 EMU = 1 inch).
        """
        img_path = Path(image_path)
        ...
        try:
            ...
            aspect = h / w
            if aspect > 0:
                height_limited_width = int(max_height_emu / aspect)
                if height_limited_width > 0:
                    target_width = min(int(max_width_emu), height_limited_width)
        except Exception:
            target_width = int(max_width_emu)
...
class S32DocxFiller(_DocxHelper):
    """
    Fills QOS section 2.3.S.3.2 (i) API-related impurities.

    - Extracts API-related impurity table(s) from the 3.2.S.3.2 PDF using PyMuPDF table detection.
    - Writes name/origin as text into the template DOCX placeholder table.
    - Crops and inserts the structure graphic as an image into the Structure cell.
    """
    ...
    def _find_section_pages(
        ...
        *,
        ...
        start_re = re.compile(self._section_flexible_regex(refer_section), re.IGNORECASE)
        start_page = None
        for i in range(doc.page_count):
            txt = doc.load_page(i).get_text("text", sort=True)
            if start_re.search(txt):
                start_page = i
                break
        ...
        if next_section:
            next_re = re.compile(self._section_flexible_regex(next_section), re.IGNORECASE)
            for i in range(start_page + 1, doc.page_count):
                txt = doc.load_page(i).get_text("text", sort=True)
                if next_re.search(txt):
                    end_page = max(start_page, i - 1)
                    break
        return (start_page, end_page)

    @staticmethod
    def _norm_cell_text(s) -> str:
        if s is None:
            return ""
    ...
    def _extract_api_impurity_rows(
        ...
        *,
        ...
        if self.images_dir is None:
            return ([], ["images_dir not configured; cannot extract structure images"])
        ...
        def is_api_table(tab_rows: list[list[object]]) -> bool:
            window = tab_rows[:4]
            flat = " ".join(
                self._norm_cell_text(c) for r in window for c in (r[:3] if r else [])
            ).lower()
                    ...
                    for tab in found.tables:
                        ...
                        for ridx, row_text in enumerate(extracted):
                            if ridx >= len(tab.rows):
                                break
                            c0 = self._norm_cell_text(row_text[0] if len(row_text) > 0 else "")
                            c2 = self._norm_cell_text(row_text[2] if len(row_text) > 2 else "")
                ...
                for i, e in enumerate(entry_specs, start=1):
                    ...
                    if rect is not None:
                        ...
                        if not cell_rect.is_empty:
                            ...
                            inner = fitz.Rect(
                                cell_rect.x0 + inset_x,
                                cell_rect.y0 + inset_y,
                                cell_rect.x1 - inset_x,
                                cell_rect.y1 - inset_y,
                            ) & cell_rect
    ...
    def fill_s32_section(
        ...
        start_idx, end_idx = self._get_target_range(doc)
        anchor_idx = self._find_para_index_doc(
            doc,
            "List of API-related impurities",
            start_idx,
            end_idx,
        )
        if anchor_idx is None:
            ...
            if placeholder is None:
                ...
                try:
                    ...
                    WD_CELL_VERTICAL_ALIGNMENT = None  # type: ignore
                ...
                for i, row in enumerate(rows, start=1):
                    ...
                    if img and isinstance(img, Path) and img.exists():
                        ...
                        try:
                            ...
                            self._add_picture_autofit_bounds(
                                run,
                                img,
                                max_width_emu=max_w,
